...
translatePClauseAsCase_ trans (MkPatClause _ lhs rhs _) = AST.MkCaseClause (trans lhs) (trans rhs)
translatePClauseAsCase_ trans (MkWithClause _ lhs wps _ _) = AST.MkCaseClause (trans lhs) (AST.EComment (MkComment LineComment "with clause" 0 0) (AST.EImplicit))
...
translatePFieldUpdate_ trans (PSetField path v) = AST.EComment (MkComment LineComment ("set " ++ show path) 0 0) (trans v)
translatePFieldUpdate_ trans (PSetFieldApp path v) = AST.EComment (MkComment LineComment ("setApp " ++ show path) 0 0) (trans v)
...
translatePDo_ trans (DoBind _ _ n rig ty tm) = AST.DoBind (translateName n) (translateRig rig) (map trans ty) (trans tm)
translatePDo_ trans (DoBindPat _ pat ty val alts) = AST.DoBindPat (trans pat) (map trans ty) (trans val) (map (translatePClauseAsCase_ trans) alts)
translatePDo_ trans (DoLet _ _ n rig ty val) = AST.DoLet (translateName n) (translateRig rig) (trans val)
translatePDo_ trans (DoLetPat _ pat ty val alts) = AST.DoLetPat (trans pat) (trans val) (map (translatePClauseAsCase_ trans) alts)
translatePDo_ trans (DoLetLocal _ decls) = AST.DoExp (AST.EComment (MkComment LineComment "local decls in do" 0 0) AST.EImplicit)
...
mutual
  ...
  extractPiParams (PPi _ _ _ (Just n) arg ret) = let (ps , ty) = extractPiParams ret
  ...
  translatePlainBinder pb = let n : WithFC CN.Name = WithData.get "name" pb
  ...
  translateBasicMultiBinder (MkBasicMultiBinder rig names ty) = map (\n => (translateName (val n) , translatePTerm ty)) (list1ToList names)
  ...
  translatePClauseAsCase (MkPatClause _ lhs rhs _) = AST.MkCaseClause (translatePTerm lhs) (translatePTerm rhs)
  translatePClauseAsCase (MkWithClause _ lhs wps _ _) = AST.MkCaseClause (translatePTerm lhs) (AST.EComment (MkComment LineComment "with clause" 0 0) (AST.EImplicit))
  ...
  translatePTerm (PPi _ rig pii mn arg ret) = AST.EPi (translateRig rig) (translatePiInfo pii) (map translateName mn) (translatePTerm arg) (translatePTerm ret)
  translatePTerm (PLam _ rig pii pat ty scope) = AST.ELam (translateRig rig) (translatePiInfo pii) (translatePTerm pat) (translatePTerm ty) (translatePTerm scope)
  translatePTerm (PApp _ f x) = AST.EApp (translatePTerm f) (translatePTerm x)
  translatePTerm (PWithApp _ f x) = AST.EWithApp (translatePTerm f) (translatePTerm x)
  translatePTerm (PNamedApp _ f n x) = AST.ENamedApp (translatePTerm f) (translateName n) (translatePTerm x)
  translatePTerm (PAutoApp _ f x) = AST.EAutoApp (translatePTerm f) (translatePTerm x)
  translatePTerm (PPostfixApp _ rec fields) = AST.EPostfixApp (translatePTerm rec) (map (translateName . snd) fields)
  translatePTerm (PPostfixAppPartial _ fields) = AST.EPostfixAppPartial (map (translateName . snd) fields)
  translatePTerm (PPrefixOp _ op x) = AST.EPrefixOp (translateOpStr op.val) (translatePTerm x)
  translatePTerm (PSectionL _ op x) = AST.ESectionL (translateOpStr op.val) (translatePTerm x)
  translatePTerm (PSectionR _ x op) = AST.ESectionR (translatePTerm x) (translateOpStr op.val)
  translatePTerm (PEq _ l r) = AST.EOp (translatePTerm l) (AST.OpSymbols "=") (translatePTerm r)
  ...
  translatePTerm (POp _ lhsInfo op rhs) = AST.EOp (translatePTerm lhsInfo.val.getLhs) (translateOpStr op.val) (translatePTerm rhs)
  ...
  translatePTerm (PIfThenElse _ c t f) = AST.EIf (translatePTerm c) (translatePTerm t) (translatePTerm f)
  translatePTerm (PIdiom _ ns x) = AST.EIdiom (map show ns) (translatePTerm x)
  translatePTerm (PCase _ _ scrut alts) = AST.ECase (translatePTerm scrut) (map (translatePClauseAsCase_ translatePTerm) alts)
  translatePTerm (PDoBlock _ ns stmts) = AST.EDo (map show ns) (map (translatePDo_ translatePTerm) stmts)
  translatePTerm (PUpdate _ updates) = AST.EList (map (translatePFieldUpdate_ translatePTerm) updates)
  translatePTerm (PRewrite _ rule tm) = AST.EComment (MkComment LineComment "rewrite" 0 0) (translatePTerm tm)
  translatePTerm (PComprehension _ tm stmts) = AST.EComment (MkComment LineComment "comprehension" 0 0) (translatePTerm tm)
  translatePTerm (PRange _ start step end) = AST.EComment (MkComment LineComment "range" 0 0) (translatePTerm start)
  translatePTerm (PRangeStream _ start step) = AST.EComment (MkComment LineComment "range stream" 0 0) (translatePTerm start)
  translatePTerm (PLet _ rig pat ty val scope alts) = AST.ELet (translateRig rig) (translatePTerm pat) (translatePTerm ty) (translatePTerm val) (translatePTerm scope) (map (translatePClauseAsCase_ translatePTerm) alts)
  translatePTerm (PLocal _ decls scope) = AST.ELocal [] (translatePTerm scope)
  translatePTerm (NewPi x) = let binder = x.val.binder
  ...
  translatePTerm (PMultiline _ _ _ _) = AST.EComment (MkComment LineComment "multiline string" 0 0) AST.EImplicit
...
translatePTypeDecl pty = let td = val pty
...
mutual
  ...
  translatePClause (MkPatClause _ lhs rhs ws) = AST.MkClause (translatePTerm lhs) (translatePTerm rhs) (map (snd . translatePDecl) ws)
  translatePClause (MkWithClause _ lhs wps _ _) = AST.MkClause (translatePTerm lhs) (AST.EComment (MkComment LineComment "with clause" 0 0) (AST.EImplicit)) []
  ...
  translatePDecl pdecl = let line = fcLine pdecl.fc
                         in let d = val pdecl
                            in pair line $ case d of
                                             IS.PClaim claim => let tyDecl = val claim.type
                                                                   ...
                                                                   in let docs = docToComments tyDecl.doc
                                                                      in let vis = translateVisibility claim.vis
                                                                         in case names of
                                                                              ...
                                                                              (n :: _) => AST.DClaim docs vis (translateName n) (translatePTerm tyDecl.type) (map translateFnOpt claim.opts)
                                             IS.PDef clauses => case clauses of
                                                                  [] => AST.DComment (MkComment LineComment "empty PDef" 0 0)
                                                                  (c :: _) => let lhs = case c of
                                             ...
                                             IS.PData doc vis treq (MkPData _ tyname tycon opts datacons) => let (params , ty) = translateDataType tycon
                                             ...
                                             IS.PData doc vis treq (MkPLater _ tyname tycon) => AST.DComment (MkComment LineComment "forward data declaration" 0 0)
                                             IS.PParameters params decls => let ps = case params of
                                             ...
                                             IS.PUsing usings decls => let us = map (\(mn , tm) => (map translateName mn , translatePTerm tm)) usings
                                                                       in AST.DUsing us (map (snd . translatePDecl) decls)
                                             IS.PInterface vis constraints name doc params det conName methods => let ps = concatMap translateBasicMultiBinder params
                                             ...
                                             IS.PImplementation vis opts pass implicits constraints name params implName nusing body => let visibility = translateVisibility vis
                                             ...
                                             IS.PRecord doc vis treq (MkPRecord tyname params opts conName decls) => let ps = concatMap (translateBasicMultiBinder . bind) params
                                             ...
                                             IS.PRecord doc vis treq (MkPRecordLater tyname params) => AST.DComment (MkComment LineComment "forward record declaration" 0 0)
                                             IS.PFail msg decls => AST.DComment (MkComment LineComment "PFail not yet translated" 0 0)
                                             IS.PMutual decls => AST.DMutual (map (snd . translatePDecl) decls)
                                             IS.PFixity fixData => let ops = map (\op => translateName op.toName) (forget fixData.operators)
                                                                   in AST.DFixity (MkFixityDecl (translateFixity fixData.fixity) fixData.precedence ops)
                                             IS.PNamespace ns decls => AST.DNamespace [show ns] (map (snd . translatePDecl) decls)
                                             IS.PTransform name lhs rhs => AST.DTransform name (translatePTerm lhs) (translatePTerm rhs)
                                             IS.PRunElabDecl tm => AST.DRunElab (translatePTerm tm)
                                             IS.PDirective dir => AST.DDirective (translateDirective dir)
                                             IS.PBuiltin bt n => AST.DBuiltin (show bt) (translateName n)
...
extractText src (sl , sc) (el , ec) = let ls = lines' src
  ...
  where
    ...
    extractMulti first rest sc' sl' el' ec' = let f = pack (drop sc' (unpack first))
...
extractComments src state = let decs = state.decorations
...
mergeByLine [] ys = ys
mergeByLine xs [] = xs
mergeByLine ((lx , x) :: xs) ((ly , y) :: ys) = if lx <= ly
...
insertBlanks [] = []
insertBlanks [(_ , d)] = [d]
insertBlanks ((l1 , d1) :: (l2 , d2) :: rest) = if l2 > l1 + 1
...
parseModule src = let origin = CFC.Virtual CFC.Interactive
                  in let result = PS.runParser origin Nothing src (IP.prog origin)
                     in case result of
                          Left err => Left (fromError err)
                          Right (_ , (state , mod)) => let modName = show (IS.Module.moduleNS mod)
                                                             ...
                                                             in let decls = map translatePDecl (IS.Module.decls mod)
                                                                in let comments = map commentPair (extractComments src state)
                                                                   in let merged = mergeByLine (header :: imps ++ decls) comments
                                                                      in let withBlanks = insertBlanks merged
                                                                         in Right withBlanks
