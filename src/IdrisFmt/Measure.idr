...
export
measureWidth : {opts : _} -> Doc opts -> Nat
measureWidth doc =
  let rendered = Core.Doc.render opts doc
      ls = lines rendered
   in case ls of
        [] => 0
        (l :: _) => length l
...
export
maxWidth : List Nat -> Nat
maxWidth [] = 0
maxWidth (x :: xs) = foldl max x xs
...
export
padTo : {opts : _} -> Nat -> Doc opts -> Doc opts
padTo target doc =
